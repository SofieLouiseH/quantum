...
def qsolve(config, qubo, runs):
    ...
    answer_quant = Qubo(config, qubo).with_platform("dwave").with_solver("Advantage_system6.1").with_params()
    ...
    #datei = open('favEmbedding.txt', 'r')
    #answer_quant.embedding = datei.read()
    answer_quant = Qubo(config, qubo).with_platform("dwave").with_solver("Advantage_system6.1").with_params().solve(runs)
